...
class Edit extends LoadDataTabContainer {
  ...
  tabList = [
    {
      key: 'basicInfo',
      show: this.checkAuthority(accessWayCollection.advertisement.get),
      tab: '基本信息',
    },
  ];
  ...
  supplementLoadRequestParams = o => {
    const d = o;
    const { advertisementId } = this.state;
  ...
  pageHeaderContent = () => {
    ...
    return (
      <Descriptions className={styles.headerList} size="small" col="2">
        <Description label="标识">{metaData === null ? '' : metaData.advertisementId}</Description>
        <Description label="类别">{metaData === null ? '' : metaData.className}</Description>
