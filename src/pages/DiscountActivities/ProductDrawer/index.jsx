...
class Index extends PagerDrawer {
  componentAuthority = accessWayCollection.discountActivities.listProduct;
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      ...{
        tableScroll: { x: 1220 },
        loadApiPath: 'discountActivities/listProduct',
  ...
  doOtherWhenChangeVisible = (preProps, preState, snapshot) => {
    ...
    if (!firstLoadSuccess) {
      // 设置界面效果为加载中，减少用户误解
      this.setState({ dataLoading: true, selectedItem: null });
  ...
  selectRecord = (e, record) => {
    this.setState({ selectedItem: record }, () => {
      ...
      if (isFunction(afterSelectSuccess)) {
        ...
        data.goodsTitle = record.title;
        data.goodsType = 1;
        data.businessId = record.productId;
